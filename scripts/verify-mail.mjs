 ...
 *
 * Answers one question: will the quote form be able to deliver mail with the
 * current credentials? It authenticates against the SMTP server and stops --
 * it never sends a message, so it is safe to run against production settings.
 *
 *   npm run verify:mail
 *
 * Reads .env directly rather than through astro:env, so it works standalone
...
function loadEnv() {
	// process.env wins, so CI and Vercel-style injected vars take precedence.
	const fromFile = {};
...
/** Mask a credential so the output is safe to paste into a bug report. */
function mask(value) {
...
if (reachable !== "open") {
	// Suggest the alternative before the auth attempt, since a blocked port
	// produces a timeout that is easy to misread as a credential failure.
