...
export function explainMailError(error: unknown): string {
	...
	if (code === "EAUTH" || responseCode === 535) {
		return "SMTP rejected the credentials (535 Authentication Failed). Zoho refuses a normal account password over SMTP when two-factor auth is enabled -- generate an app-specific password (Zoho Mail > My Account > Security > App Passwords) and set ZOHO_SMTP_PASSWORD to it. Also confirm the account's datacenter: a .eu / .in / .com.au account will not authenticate against smtp.zoho.com.";
	}
	if (code === "ETIMEDOUT" || code === "ESOCKET" || code === "ECONNECTION") {
		return `Could not reach the SMTP server (${code}). The port is most likely blocked outbound on this network. Port 465 is commonly blocked; try ZOHO_SMTP_PORT=587, which uses STARTTLS.`;
	}
	if (code === "EENVELOPE") {
